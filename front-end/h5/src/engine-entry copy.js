 ...
 * @LastEditors: ly525
 * @LastEditTime: 2020-04-08 18:43:16
...
const Engine = {
  ...
  methods: {
    ...
    dispatchRequest () {
      const dsList = (window.__work.dataSources || []).filter(ds => ds.type === 'http-api')
      const requests = dsList.map(ds => axios(ds.url))
      axios
        .all(requests)
        .then(
