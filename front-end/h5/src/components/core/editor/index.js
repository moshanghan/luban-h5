...
import RenderDataSourcePanel from './data-source/index'
import PreviewDialog from './modals/preview.vue'
import ScriptEditDialog from './modals/script-edit.vue'
import ScriptViewDialog from './modals/script-view.vue'
import ScriptListDialog from './modals/script-list'
import ScriptCreateDialog from './modals/script-create.vue'
import DataSourceEditDialog from './modals/data-source-editor.vue'
...
function postMessage2Iframe (message) {
  const iframe = document.getElementById('iframe-for-preview-1')
  if (!iframe) return
  let iframeWin = iframe.contentWindow
  // iframeWin.postMessage(message, window.location.origin)
  iframeWin.postMessage(message, new URL(iframe.src).origin)
}
...
export default {
  ...
  render (h) {
    return (
      <a-layout id="luban-editor-layout" style={{ height: '100vh' }}>
          ...
          <a-layout id="canvas-outer-wrapper">
            <div class="mode-toggle-wrapper">
              <a-radio-group
                ...
                onInput={isPreviewMode => {
                  ...
                  if (isPreviewMode) {
                    postMessage2Iframe(JSON.stringify(this.work))
            ...
            <a-layout-content style={{ transform: `scale(${this.scaleRate})`, 'transform-origin': 'center top' }}>
              <div class='canvas-wrapper' style={{
                ...
                {/* { this.isPreviewMode
                  ? <RenderPreviewCanvas elements={this.elements} work={this.work} />
                  : <RenderEditCanvas
                    class="edit-mode"
          ...
          <a-layout-sider width={this.propsPanelWidth} data-set-width={this.propsPanelWidth} theme='light' style={{ background: '#fff', padding: '0 0 0 12px' }}>
            ...
            >
              ...
              */}
              ...
              <a-tab-pane label="动作" key='动作' tab={this.$t('editor.editPanel.tab.action')}><RenderActionEditor/></a-tab-pane>
        ...
        />
        <ScriptCreateDialog
          visible={this.createScript_dialog}
          handleClose={() => { this.updateDialog({ type: 'createScript_dialog', value: false }) }}
        />
        <ScriptEditDialog
          visible={this.editScript_dialog}
          handleClose={() => { this.updateDialog({ type: 'editScript_dialog', value: false }) }}
        />
        <ScriptViewDialog
          zIndex={1001}
  ...
  created () {
    ...
    // setInterval(() => {
    //   this.updateStorage(Math.random())
    // }, 200)
  }
}
